      ...
      return {
        ...
        menus: [
          ...
          },
          {
            ...
            path: `schedules/${this.$store?.state?.user?.latestAcademicRecord.id}`,
            isVisible: true
          },
          {
            title: 'My Ledger',
...
<style lang="scss" scoped>
  ...
  .home__menus {
    width: auto;
    max-width: calc((230px + 15px + 15px) * 4); // size of each menu plus 10px from left and 10px from right times number of columns
    display: flex;
    flex-direction: row;
    flex-wrap: wrap;
    margin-top: 30px;
