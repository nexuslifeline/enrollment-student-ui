...
export default {
        ...
        if (student.evaluation.evaluationStatusId === EvaluationStatuses.APPROVED.id) {
          ...
          const data = {
            activeAdmission: {
              ...
              admissionStepId : AdmissionSteps.REQUEST_EVALUATION.id
            }
          }
    ...
    methods: {
        ...
        this.getBillingList(params).then(({ data }) => {
          ...
          if (data[0].payments[0] != null) {
            ...
            if (payment.fields.paymentStatusId === PaymentStatuses.APPROVED.id) {
              const { activeAdmission, academicRecord, student } = this.forms
              const admission = {
                activeAdmission: {
      ...
      filterSubject() {
        ...
        if (levelId !== null && semesterId !== null) {
            subjects.filteredItems = subjects.items.filter(
              s => s.pivot.levelId === levelId && s.pivot.semesterId === semesterId
            )
        } else if (levelId !== null && semesterId === null) {
          subjects.filteredItems = subjects.items.filter(
            s => s.pivot.levelId === levelId
          )

        } else if (semesterId !== null && levelId === null) {
          subjects.filteredItems = subjects.items.filter(
            s => s.pivot.semesterId === semesterId
          )
        } else {
      ...
      prePopulateStudentSubjects() {
        // const { evaluation, academicRecord } = this.forms
        ...
        //       subject.pivot.levelId === academicRecord.fields.levelId &&
        //       subject.pivot.semesterId === academicRecord.fields.semesterId
        //     );
